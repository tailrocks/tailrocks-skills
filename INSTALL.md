...
[v0.19.0](https://github.com/tailrocks/tailrocks-skills/releases/latest).
...
|---|---|---|---|
| Claude Code | **Yes**, per marketplace — off by default for third-party marketplaces, turn it on once | `/plugin marketplace update tailrocks-skills` | `#v0.19.0` on the Git URL |
| Codex CLI | No | `codex plugin marketplace upgrade tailrocks-skills`, then `codex plugin add tailrocks-skills` | `@v0.19.0` or `--ref v0.19.0` |
| Grok Build | No | `grok plugin update tailrocks-skills` | `@v0.19.0` |
...
| OpenCode | No — it is a file copy | re-copy the tree | `git clone --branch v0.19.0` |
| Antigravity CLI | No — it installs from a local path | re-clone, reinstall | `git clone --branch v0.19.0` |
...
```sh
claude plugin marketplace add https://github.com/tailrocks/tailrocks-skills.git#v0.19.0
```
...
```sh
codex plugin marketplace add tailrocks/tailrocks-skills@v0.19.0
```
...
```sh
git clone --depth 1 --branch v0.19.0 https://github.com/tailrocks/tailrocks-skills.git /tmp/tailrocks-skills
...
tailrocks/tailrocks-skills@v0.19.0 --trust`.

- Grok resolves `.grok-plugin/plugin.json` first, then falls back to
  `.claude-plugin/plugin.json` — this repo installs as-is. `@v0.19.0` ref
...
release URL (`.../releases/tag/v0.19.0`) or a `/tree/<ref>` URL, and repeat the
...
```sh
git clone --depth 1 --branch v0.19.0 https://github.com/tailrocks/tailrocks-skills.git
