...
- Duplicate semantics: Codex dedupes by SKILL.md **path**, never by name —
  ...
  is deprecated.)
- This install has marketplace name equal to plugin name
  (`tailrocks-skills@tailrocks-skills`), the precondition of an open Codex
  path-flattening bug (openai/codex#35648) where the client may drop a path
  segment when loading skill files; if a skill file or one of its references
  fails to load with a path missing a directory segment, that is the client,
  not the install.
