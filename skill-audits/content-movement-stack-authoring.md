...
|---|---:|---|---|
| `shared/references/runtime-trust.md` | COPY | Every declared destination in `generated-references.json`, including the four active skill-authoring owners | Generator manifest is exhaustive; byte equality and no extra local edits are validated. No removed migration-product destination exists. |
...
| Eval ID `1` | PRESERVE | Frozen legacy path, inactive and undiscoverable | Direct split-route proof lives in non-protected deterministic ownership tests; no migration executor route exists. |
...
| Eval ID `1` | PRESERVE | Frozen legacy path, inactive and undiscoverable | Composed-route proof lives in non-protected deterministic ownership tests; no migration executor route exists. |
...
| Contract-delta branch and durable handoff | DELETE | No artifact or product route | Refactor stops unchanged and reports the exact delta, compatibility, rollback, and separately scoped direct-migration authority in conversation only. |
| Current stop/do-not-execute boundary | KEEP | `skills/tailrocks-skill-refactor/SKILL.md` and `skills/tailrocks-skill-update/SKILL.md` | Neither owner executes direct migration under any selector or inherited authority. |
...
For every section above, retain the base metadata/generated README/catalog entry when the base remains. Generate each new skill's `agents/openai.yaml`, README/docs page, catalog/installation/choosing entry, and non-protected deterministic evidence. Retired public skill surfaces are removed directly; no deprecated aliases, dispatchers, compatibility routes, or migration-product owners remain.
