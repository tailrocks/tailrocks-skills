...
test("root, install, choosing, and context doctrine agree on counts and authority", async () => {
  ...
  expect(install).toContain("other 64 skills are");
  ...
  expect(budget).toContain("current 64 manual-only skills");
  expect(budget).toContain("11 model-policy descriptions");
  expect(budget).toContain("all 75 descriptions");
  expect(budget).toContain("3,456 across the 64 manual owners");
  expect(budget).toContain("current 75 descriptions total 19,306 characters");
  expect(budget).toContain("15,786 count against the per-skill caps");
