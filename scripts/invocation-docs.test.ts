...
test("root, install, choosing, and context doctrine agree on counts and authority", async () => {
  ...
  expect(install).toContain("other 41 skills are");
  ...
  expect(budget).toContain("current 41 manual-only skills");
  expect(budget).toContain("11 model-policy descriptions");
  expect(budget).toContain("all 52 descriptions");
  expect(budget).toContain("2,214 across the 41 manual owners");
  expect(budget).toContain("current 52 descriptions total 13,644 characters");
  expect(budget).toContain("11,389 count against the per-skill caps");
