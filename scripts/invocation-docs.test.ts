...
test("root, install, choosing, and context doctrine agree on counts and authority", async () => {
  ...
  expect(install).toContain("other 55 skills are");
  ...
  expect(budget).toContain("current 55 manual-only skills");
  expect(budget).toContain("11 model-policy descriptions");
  expect(budget).toContain("all 66 descriptions");
  expect(budget).toContain("2,970 across the 55 manual owners");
  expect(budget).toContain("current 66 descriptions total 17,252 characters");
  expect(budget).toContain("14,227 count against the per-skill caps");
