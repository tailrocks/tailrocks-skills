# Plan 029: Publish macOS arm64 provider evidence
...
Provider support is not established by the qualification package alone. Exact
released bytes must reproduce every advertised macOS arm64 row on a physical
native host. The host observation stays `operator_attested`; protected main
validates its subject closure and publishes directly attested OCI evidence.
...
TAILROCKS_PROVIDER_VERIFY_TMP="$(mktemp -d /tmp/tailrocks-provider-macos.XXXXXX)"
...
Expected: clean detached sole merge, exact merged-PR/main readback, native
Darwin arm64, one immutable plan-018 release, and one exact digest-only
plan-023 qualification subject.

## Spec contract

### Requirement G06/G14/G15: native provider evidence outside Git

The canonical `provider-platform-evidence-v1` record SHALL bind atomic merge/PR,
release/tag/manifest/index/arm64 child, canonical `native-boundary-v1` object/
SHA-256, exact provider-qualification OCI digest,
policy/protected-proof identities carried by that qualification, provider client
version/origin/config/isolation, independent row verdicts, lifecycle/exit
classes, trust labels, and actual run date. Host/provider observations remain
`operator_attested`. `publish-goal-evidence.yml` SHALL reverify every
machine-checkable release/qualification fact, validate the exact submitted
bytes/digest, publish to `tailrocks-provider-platform-evidence`, and directly
attest file and OCI subjects from protected main.
`evidence-manifest.json` remains a derived local publication receipt; it is not
inside the OCI payload and is never the published OCI subject.

#### Scenario: one provider regresses or cannot run

- **WHEN** auth/TTY/isolation is absent, client origin/version/config drifted, or
  any row is lower, failing, or inconclusive
- **THEN** record that exact result; publication may retain evidence but plan 042
  cannot advertise the stronger row.
...
- **N06/N07/N17**: registry auth is removed before provider children; provider
  auth never enters evidence; autonomous tiers require the recorded isolation.
- **N11**: no Git/release/qualification/docs mutation or moving OCI alias.
- **N16/N18**: provider runs, output, tools, JSON, time, and integrity-only query
  budget are closed and bounded; no confidentiality claim.
...
- Per-provider current auth/TTY in the task-only home of the distinct
  `tailrocks-native-client` principal. Missing isolation/input produces an
  honest lower/INCONCLUSIVE row.
...
- The atomic merge contains every adapter/validator, generic static docs,
  `publish-goal-evidence.yml`, and the closed provider publication interface.
- Plan 023 published qualification metadata for the existing plan-018 release;
  it did not create a second software version or tag.
...
tailrocks_cleanup_provider_macos_auth() {
  set +e
...
trap tailrocks_cleanup_provider_macos_auth EXIT
...
TAILROCKS_NATIVE_BROKER="$TAILROCKS_PROVIDER_VERIFY_TMP/release/tailrocks-native-client-broker-aarch64-apple-darwin"
...
qualification mutation, docs/date/version edit, Linux fan-in, policy/proof work.
...
Verify all release assets/provenance and exact qualification OCI/file
attestations under task-private registry auth. Materialize only validated
canonical qualification JSON into the task root, then log out/remove registry
auth before starting any provider child.
...
Run each applicable exact provider client/config through the released binary on
the physical host and recorded isolation boundary, one provider per fresh
broker lease/task home with full cleanup and quiescence before the next. Preserve each independent
TIER 0/1/2/INCONCLUSIVE result, lifecycle state, budget, origin, and limitation.

**Verify**: wrong target/child/client/origin/config/isolation, leaked auth,
unbounded tool/output, tier promotion, hidden regression, or transcript-derived
PASS fails validation or records the honest lower result.
...
schema/bound checks, preserves operator trust labels, and directly attests file
and OCI output. Re-authenticate only for digest pull/attestation verification,
then clean auth. Re-read main/PR and require detached clean HEAD.

**Verify**: publication helper binds purpose/schema/input digest/qualification/
atomic SHA; attestation pins repository, workflow, main, source+signer SHA, and
hosted runner. Output is digest-only; final Git/PR/main state equals preflight.
...
- [ ] Every applicable macOS arm64 provider row has an exact honest result.
...
Plan 042 consumes this exact OCI URI with plan 041's independent Linux URI and
