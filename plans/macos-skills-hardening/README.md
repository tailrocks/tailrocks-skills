...
|------|------|-------|--------|----------|
| W1 — Verify every Apple claim in all six `references/` dirs (API signatures, availability strings, quoted HIG sentences) via DocC JSON; correct or delete failures. Verify: citation ledger (`w1-citation-ledger.md`) lists every factual claim with source URL and VERIFIED or DELETED, none unresolved. | L | all six skills | DONE | `w1-citation-ledger.md`: ~120 claim rows across 26 reference files, all VERIFIED/CORRECTED/DELETED; 60+ swiftc probes (SDK 26.5), 17 negative probes, ~35 DocC JSON fetches, Landmarks zip + Apple macOS 27 UI Kit re-measured; commit 2b03eed |
...
| W3 — Generate scratch app outside repo from `templates/project.yml` + `mise.toml`; build, test, launch, capture with `templates/capture.sh`, run `performAccessibilityAudit`. Fix templates on failure. Skip `swift-format.json` and `window-id.swift`. Verify: transcript shows every step succeeding and captured PNG inspected (not merely written). Needs GUI session with Screen Recording, Accessibility, Automation. | M | swift-project-setup + macos-visual-qa templates | DONE | `w3-scratch-app-transcript.md`: generate/build/test/launch/capture/audit all green (final `mise run test` exit 0); PNG and audit element screenshots visually inspected; 1 template defect fixed (test-target Info.plist), 2 skill-prose fixes from exercised failures |
| W4 — Run all 24 eval cases across the six `evals/evals.json` files. Fix skill prose where a case fails; never weaken an `expected_output`. Verify: 24 rows all PASS, each prose change traceable to a failed case. | M | all six skills | DONE | `w4-eval-results.md`: 24/24 PASS across rounds 1–5 with per-case fix traceability; no expected_output weakened; provenance reruns on final merged routers |
| W5 — Add worked AppKit patterns to `tailrocks-liquid-glass/references/appkit-api.md`: glass container cluster, background extension view, prominent tinted toolbar item, scroll edge effect style. Verify: each compiles with `swiftc` and names its SDK. | M | liquid-glass | DONE | 4 worked patterns added (glass cluster, background extension, prominent tinted toolbar item, scroll edge style); each block extracted verbatim from appkit-api.md and compiled: `swiftc -c -target arm64-apple-macos26.0`, macOS 26.5 SDK, doc-block-0..3 all OK; each names its SDK in prose |
| W6 — Dogfood once on one real screen: brief, component map, alternatives, implementation, rendered state matrix, rubric score. Every stall is a skill defect with a skill commit. Verify: artifacts exist, rubric scored against captures not source. Needs GUI session with Screen Recording, Accessibility, Automation. | L | all six skills | DONE | `w6-dogfood/`: brief, map, contract, alternatives, implementation, 9 captures, StateMatrix with recorded skips, independent DesignReview scored from captures (rejected 57/100 — the loop catching real defects), ReviewDisposition; skill defects committed in a7574db + 3adb281 |
| W7 — INSTALL.md and `docs/` omit the macOS family; add them. Verify: validator green, every new skill name in INSTALL.md. | S | INSTALL.md, docs/ | DONE | INSTALL.md "What installs" inventory names all six (grep count 1 each); `mise run validate` → "Validated 21 skills.", exit 0. docs/ audited: four feature design docs, no skill catalog surface, no stale claims — README.md already lists the family |
| W8 — Add dated external-skill ledger to `tailrocks-swift-project-setup/references/agent-integration.md`: stars, license, macOS coverage, native-vs-web classification; note Apple's exported skills contain no Liquid Glass skill and no macOS skill; mark unlicensed repos read-only/never vendorable. Verify: every URL resolves, each license matches what the repo publishes. | S | agent-integration.md | DONE | Reworked 2026-08-11 per maintainer direction: shipped skill content carries distilled findings only, no external skill references; raw 13-row survey (all URLs HTTP 200, licenses verified via GitHub API/raw LICENSE) preserved as internal evidence in `w8-external-survey.md` |
