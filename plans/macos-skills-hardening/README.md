...
| W4 — Run all 24 eval cases across the six `evals/evals.json` files. Fix skill prose where a case fails; never weaken an `expected_output`. Verify: 24 rows all PASS, each prose change traceable to a failed case. | M | all six skills | IN PROGRESS | — |
