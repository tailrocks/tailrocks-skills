...
Review of the executor's diff happens on the capable tier, against the plan's
own done criteria and out-of-scope list — never on the executor's "done" claim.
Two send-back rounds maximum; a third failure means the plan is wrong, not the
executor. Merging always stays the user's call.
