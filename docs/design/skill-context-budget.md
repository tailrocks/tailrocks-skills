...
Consequence: in Claude Code and Codex the current 41 manual-only skills add
...
both fields and load all 52 descriptions; there the explicit-request guard on
the 41 manual owners is their discovery control.

## Measurement: the guard sentence earns its length

The guard costs 54 characters per skill, 2,214 across the 41 manual owners.
Whether a
...
else is the router's job. The current 52 descriptions total 13,644 characters;
11,389 count against the per-skill caps. The other 2,255 are the 2,214 guard
characters plus one trimmed separator on each of 41 manual owners.
