...
Consequence: in Claude Code and Codex the current 44 manual-only skills add
...
both fields and load all 55 descriptions; there the explicit-request guard on
the 44 manual owners is their discovery control.

## Measurement: the guard sentence earns its length

The guard costs 54 characters per skill, 2,376 across the 44 manual owners.
Whether a
...
else is the router's job. The current 55 descriptions total 14,432 characters;
12,012 count against the per-skill caps. The other 2,420 are the 2,376 guard
characters plus one trimmed separator on each of 44 manual owners.
