...
Contract-breaking migration is a separate, operator-approved workflow recorded
in `skill-migrations/<slug>.md`. Refactor may write that handoff, then stops; no
authoring skill executes it.
...
| [`tailrocks-skill-refactor`](/docs/skills/tailrocks-skill-refactor) | Ownership structure must change while behavior and public contracts stay frozen. Contract-breaking work gets a durable migration handoff; audit remains a separate manual invocation. |
