...
| Skill | Reach for it when |
|---|---|
| [`tailrocks-skill-create`](/docs/skills/tailrocks-skill-create) | A capability needs a new skill — baseline the observed failure, decide placement, match the guidance form, wire the repository. |
| [`tailrocks-skill-update`](/docs/skills/tailrocks-skill-update) | An existing skill's behavior must change in place — eval-pinned lines protected, the invocation contract untouched. |
...
| [`tailrocks-skill-refactor`](/docs/skills/tailrocks-skill-refactor) | Selected findings from an audit report must be applied — by ID, nothing beyond the selection, verified by an independent re-audit. |
...
routers, references, and evals. The placement decision is part of
...
— the router budget, the load-bearing lines, the full eval re-run — and its
...
**skills** — descriptions, routers, references, evals, wiring, overlap —
...
| [`tailrocks-merge-pr`](/docs/skills/tailrocks-merge-pr) | A PR is merge-ready and the gates must run fail-closed before it lands — including the read-only delivery-artifact check when the diff touches `roadmap/`. |
...
touches `roadmap/`, its delivery-artifact check reads that diff for five
...
verification round still carried a blocking defect. It never edits, deletes,
