...
### tui-design and the other design owners

`tailrocks-tui-design` owns terminal screens only. macOS windows keep their
taste owner ([`tailrocks-macos-design`](/docs/skills/tailrocks-macos-design));
a design skill for one medium never rules on another.
