...
| [`tailrocks-reconcile`](/docs/skills/tailrocks-reconcile) | Execution status, blockers, drift, and DONE claims need re-verifying against the repository, or a verification round needs closing. |
...
defects, and is the only skill that may set `DONE`.
...
it through, never touching the item.
