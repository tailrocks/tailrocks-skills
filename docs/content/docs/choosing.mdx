...
[`tailrocks-skill-author`](/docs/skills/tailrocks-skill-author) owns the edit
...
instead of from a baseline run.

Use retrospect when the question is *what went wrong across a delivery*. Use
skill-author when the question is *what should this skill say*. The whole
loop — including how a pull request in another repository becomes evidence
through `--repo`/`--pr`, and why the analysis fans out to subagents while the
judgment stays put — is documented under
[self-improvement](/docs/self-improve).
