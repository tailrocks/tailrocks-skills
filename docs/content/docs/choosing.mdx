...
The macOS family takes a feature from direction through independent review,
running proof, implementation, and verified pixels.

```mermaid
flowchart LR
  design["macos-design"] --> review["macos-design-review"]
  review --> design
  design --> baseline["macos-visual-baseline"]
  baseline --> swift["swift-best-practices"]
  swift --> gates["swift-project-setup gates"]
  gates --> qa["macos-visual-qa"]
  qa --> regression["macos-visual-regression"]
```

| Skill                                                                                   | Owns                                                                                                                                                                                                      |
| --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`tailrocks-macos-design`](/docs/skills/tailrocks-macos-design)                         | Sole taste/material owner: brief, information architecture, native component map, alternatives, selected direction, and signed-off Liquid Glass prototype. Never scores its own work or edits production. |
| [`tailrocks-macos-design-review`](/docs/skills/tailrocks-macos-design-review)           | Independently review a direction preliminarily or score the full running prototype for acceptance. Writes only the report; never fixes, blesses, captures, or systematizes.                               |
| [`tailrocks-macos-design-systematize`](/docs/skills/tailrocks-macos-design-systematize) | Persist reusable product design-system learning only after user approval and independent acceptance. Never designs, reviews, blesses, captures, or edits production.                                      |
...
| [`tailrocks-macos-visual-baseline`](/docs/skills/tailrocks-macos-visual-baseline)       | Freeze the complete separately reviewed and user-blessed prototype matrix into one reproducible baseline package.                                                                                         |
| [`tailrocks-macos-visual-qa`](/docs/skills/tailrocks-macos-visual-qa)                   | Inspect and judge the current running app through window-ID capture, accessibility driving/audit, and the restored state matrix.                                                                          |
| [`tailrocks-macos-visual-regression`](/docs/skills/tailrocks-macos-visual-regression)   | Compare current running-window captures against one exact baseline without changing project or baseline bytes.                                                                                            |
...
| -------- | -------------------------------- | --------------------------------------------------------------- | ------------------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| Terminal | Rust, ratatui, crossterm         | [`tailrocks-tui-design`](/docs/skills/tailrocks-tui-design)     | same                            | same — the golden test                                                            | [`tailrocks-tui-design-audit`](/docs/skills/tailrocks-tui-design-audit)               |
| Web      | TanStack Start, React, shadcn/ui | [`tailrocks-web-design`](/docs/skills/tailrocks-web-design)     | same                            | [`tailrocks-web-visual-baseline`](/docs/skills/tailrocks-web-visual-baseline)     | [`tailrocks-web-design-audit`](/docs/skills/tailrocks-web-design-audit)               |
| macOS    | Swift, SwiftUI, Liquid Glass     | [`tailrocks-macos-design`](/docs/skills/tailrocks-macos-design) | same — on the running prototype | [`tailrocks-macos-visual-baseline`](/docs/skills/tailrocks-macos-visual-baseline) | [`tailrocks-macos-visual-regression`](/docs/skills/tailrocks-macos-visual-regression) |
...
| [`tailrocks-tui-design-audit`](/docs/skills/tailrocks-tui-design-audit)               | Existing gallery packages or shipped terminal screens need read-only verification against blessed golden frames, registry coverage, style checks, states, and sizes.                                      |
...
| [`tailrocks-web-design-audit`](/docs/skills/tailrocks-web-design-audit)               | Existing design routes or shipped web screens need read-only verification against their blessed component, state, theme, viewport, and accessibility contract.                                            |
| [`tailrocks-web-visual-baseline`](/docs/skills/tailrocks-web-visual-baseline)         | Freeze an explicitly blessed web design-route matrix into durable Playwright baselines; never installs the harness or compares regression. |
| [`tailrocks-web-visual-regression`](/docs/skills/tailrocks-web-visual-regression)     | Compare the current web matrix to one exact baseline read-only; never updates snapshots or approves design. |
| [`tailrocks-macos-visual-baseline`](/docs/skills/tailrocks-macos-visual-baseline)     | A finalized, independently accepted, user-blessed macOS prototype needs its complete signed-off matrix frozen reproducibly.                                                                               |
| [`tailrocks-macos-visual-qa`](/docs/skills/tailrocks-macos-visual-qa)                 | The current running macOS implementation needs visual, behavioral, state-matrix, and accessibility inspection without touching baselines.                                                                 |
| [`tailrocks-macos-visual-regression`](/docs/skills/tailrocks-macos-visual-regression) | Current macOS renders need mechanical structural/pixel comparison against one exact approved baseline.                                                                                                    |
...
[`tailrocks-web-visual-baseline`](/docs/skills/tailrocks-web-visual-baseline) for web
screens and
[`tailrocks-macos-visual-baseline`](/docs/skills/tailrocks-macos-visual-baseline) for
...
### retrospect or improve

Both end in prioritized findings, and their subjects do not overlap.
[`tailrocks-improve`](/docs/skills/tailrocks-improve) reads a repository and
finds what is wrong with **the code**, returning a verified read-only report.
[`tailrocks-review-pr`](/docs/skills/tailrocks-review-pr) owns a branch or diff.
[`tailrocks-retrospect`](/docs/skills/tailrocks-retrospect) reads
...
Improve needs no roadmap item and writes none; retrospect needs exactly one.
Selected improve findings route separately to standalone planning or roadmap
seeding.
...
the _work_: it re-runs done criteria and corrects plan rows and the item's
status in the delivering repository. `tailrocks-retrospect` never writes
there — the item is evidence — and its subject is the _skills_, not the
...
and none of these four skills executes direct migration under any selector.
...
routers, references, and durable evidence. The placement decision is part of
...
Use retrospect when the question is _what went wrong across a delivery_. Use
the family when the question is _what should this skill say_. The whole loop —
...
### improve, code-health, or review-pr

[`tailrocks-improve`](/docs/skills/tailrocks-improve) is the cold repository
entry; deep and security owners add their exact read-only proofs. A selected
LOW-risk closed finding routes to `tailrocks-improve-plan`; high-risk work or an
open product decision routes to `tailrocks-seed-roadmap`. `tailrocks-root-cause`
owns recurring defect diagnosis, `tailrocks-remediate` applies only an approved
causal contract, and `tailrocks-simplify-audit` finds redundancy inside a diff.
...
[`tailrocks-review-pr`](/docs/skills/tailrocks-review-pr) reviews one open PR,
branch, or diff against author intent and never seeds work. Run
`tailrocks-improve` to find repository work, wire
...
```mermaid
flowchart LR
  seed["seed-roadmap"] --> brainstorm --> finalize --> plan --> exec["execution"]
...
| Skill                                                                 | Reach for it when                                                                                                                                                                 |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`tailrocks-seed-roadmap`](/docs/skills/tailrocks-seed-roadmap)       | One verified high-risk finding or open product decision must become a DRAFT item.                                                                                                 |
