...
an eval re-run. Two skills own it, and neither does the other's job.

```mermaid
flowchart LR
  field["shipped item, any repository"] --> retro["tailrocks-retrospect"]
  retro --> record["retrospectives/source-slug.md"]
  record --> author["tailrocks-skill-author update"]
  author --> evals["evals updated, CI re-runs"]
```
...
## The edit side — `tailrocks-skill-author`

The record's hand-off table names, per patch, the exact next command:

<Invoke skill="tailrocks-skill-author" args="update tailrocks-plan" />
...
with new depth in references, and update the target skill's **full** eval
set. Execution of those evals is CI's job; the pull request records the
expected red-before and green-after.

Every proposed patch arrives in the same six-field anchor — target, shape,
anchor point, exact text, the eval case IDs at risk, and what it replaces —
...
- **Applying a patch inside the retrospective.** Retrospect proposes;
  skill-author applies. A patch landed without the eval update is an
