...
flowchart LR
  ...
  reconcile -->|"round with no blocking defect"| done["DONE"]
```
...
## 13. When the intent changes

Halfway through, you decide the app should tail growing files after all. Do not
edit the plan: it is fingerprinted, so a hand-edit reads as `plan-drift` at the
next gate rather than as a change of mind.
