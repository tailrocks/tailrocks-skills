...
```text
Use tailrocks-idea: a macOS app that opens huge log files fast and lets you
scrub a timeline of matches.
```
...
```text
Use tailrocks-brainstorm on log-explorer.
```
...
```text
Use tailrocks-research on log-explorer: what are the current options for
calling Rust from Swift, and how does each one handle streaming and
cancellation?
```
...
```text
Use tailrocks-research on log-explorer: what does Liquid Glass require of a
document-style macOS window, and what is the deployment-target floor for the
APIs we would use?
```
...
```text
Use tailrocks-record-decision on log-explorer: Rust owns parsing, indexing,
and query; Swift owns rendering and input only. The interface is a typed
snapshot the UI polls, never a callback into Swift, because a callback across
the boundary would put Rust threads on the main actor.
```
...
```text
Use tailrocks-record-decision on log-explorer: the app opens log files
read-only and never writes to them, because a corrupted log is unrecoverable
evidence.
```
...
```text
Use tailrocks-macos-design to design the log explorer. Stop for my
...
Optional, when the design starts in a design file rather than in prose:

```text
Use tailrocks-sketch-handoff on LogExplorer.sketch and prepare the
...
```text
Use tailrocks-finalize on log-explorer.
```
...
```text
Use tailrocks-plan on log-explorer with --deep.
```
...
The stack skills are invoked *inside* execution, named by the slice: the
executor reaches for `tailrocks-swift-best-practices` while writing the view
model, `tailrocks-liquid-glass` while building the chrome, and
`tailrocks-macos-visual-qa` to prove the result — capturing the running app by
window ID, driving the accessibility tree, and running the appearance and
accessibility state matrix. Glass surfaces snapshot fully transparent from a
detached view, so this is the only verification that means anything for the
chrome.

## 9. Reconcile — `tailrocks-reconcile`

```text
Use tailrocks-reconcile on log-explorer.
```
...
```text
Use tailrocks-record-decision on log-explorer: the app tails a growing file
and appends to the index incrementally, because operators watch live
deployments.
```
