...
1. **During shaping**, a design skill produces the artifacts the item points at.
   ...
   re-describing them. The design-reference skills go one step further and
   produce the rendered target itself — `tailrocks-tui-design` golden
   frames, `tailrocks-web-design` design routes, `tailrocks-macos-prototype`
   a runnable prototype — which each screen's `Design:` line points at and
   finalize confirms against, live. Screenshot baselines are frozen only
   after finalization (`tailrocks-web-visual-qa` for web,
   `tailrocks-macos-visual-qa` for macOS), and the plan's done criteria
   enforce the resulting check mechanically.
