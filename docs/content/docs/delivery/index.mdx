...
and assets alike, and its index row goes with it. Two commits in one
...
| [`tailrocks-reconcile`](/docs/skills/tailrocks-reconcile) | Truth, and retirement | Writing or refreshing plans | Evidence-backed statuses, the item's Remaining — and, on `DONE`, the deletion of `roadmap/<slug>/` and its index row |
...
only way to `DONE`. Inside the item's own tree it writes statuses and
