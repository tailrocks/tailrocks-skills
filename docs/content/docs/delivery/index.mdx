...
Read-only improve owners sit before the line. Use them on a repository with no
backlog; use `tailrocks-review-pr` on a branch diff. A selected LOW-risk finding
without an open product decision becomes a standalone `plans/` plan through
`tailrocks-improve-plan`. High-risk work or an open product decision enters this
line through `tailrocks-seed-roadmap` as a DRAFT item.
...
| --------------------------------------------------------------------- | ----------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`tailrocks-seed-roadmap`](/docs/skills/tailrocks-seed-roadmap)       | Verified-evidence bridge      | Low-risk standalone planning; implementation    | One DRAFT item and index row on its item lane                                                                                                                                       |
...
Every delivery skill on this page assumes you already know what you want. When
you do not, `tailrocks-improve` returns a verified read-only report;
`tailrocks-improve-deep` adds exhaustive independent refutation;
`tailrocks-improve-security` owns secret-safe threat analysis; and
`tailrocks-review-pr` owns branch-diff review. Design-conformance routes use
`tailrocks-web-design-audit`, `tailrocks-tui-design-audit`, or
`tailrocks-macos-design-review`, so discovery never invents taste.

After explicit selection, route one LOW-risk closed finding to
`tailrocks-improve-plan`, then optionally execute its standalone plan through
`tailrocks-improve-execution`. Route high-risk work or an open product decision
to `tailrocks-seed-roadmap`; the DRAFT item continues below. No combined mode
dispatches another manual skill, and no parentless delivery package exists.
...
```text
Follow roadmap/session-replay/goal/START.md
```
...
| Medium                      | Prototype built as                                                         | design                   | bless                                                  | freeze                                   | audit                               |
| --------------------------- | -------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------ | ---------------------------------------- | ----------------------------------- |
| Terminal (Rust, ratatui)    | A gallery crate rendering the application's own view functions             | `tailrocks-tui-design`   | `tailrocks-tui-design`                                 | `tailrocks-tui-design` — the golden test | `tailrocks-tui-design-audit`        |
| Web (TanStack, shadcn/ui)   | Guarded `/design/...` routes in the real app, blessed live in the browser  | `tailrocks-web-design`   | `tailrocks-web-design`                                 | `tailrocks-web-visual-baseline`          | `tailrocks-web-design-audit`        |
| macOS (Swift, Liquid Glass) | A running Liquid Glass prototype app — the material only exists at runtime | `tailrocks-macos-design` | `tailrocks-macos-design` — sign-off on the running app | `tailrocks-macos-visual-baseline`        | `tailrocks-macos-visual-regression` |
...
| You have                                                                                            | Reach for                   |
| --------------------------------------------------------------------------------------------------- | --------------------------- |
| A repository and no backlog                                                                         | `tailrocks-improve`         |
| A branch about to become a PR                                                                        | `tailrocks-review-pr`       |
