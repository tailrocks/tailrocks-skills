...
Its lanes are addressable one at a time — `security`, `perf`, `ux`,
...
own: `ux` judges against `tailrocks-web-design`'s blessed screens and
`liquid-glass` against `tailrocks-liquid-glass`'s acceptance gate, and both skip
cleanly in a repository that has neither.

<Invoke skill="tailrocks-audit" args="deep security" />
...
forcing an artifact. `execute <slug>` hands a `PLANNED` package to the cheapest
tier that can follow it, in a throwaway worktree, and reviews the resulting diff
on the capable tier against the plan's own done criteria — the plan is the
product, the diff is a proposal, and merging stays your call.
