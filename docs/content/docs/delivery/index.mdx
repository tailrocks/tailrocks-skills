...
1. **During shaping**, a design skill produces the artifacts the item points at.
   ...
   frames, `tailrocks-web-design` design routes with screenshot baselines,
   `tailrocks-macos-prototype` a runnable prototype with blessed captures —
   which each screen's `Design:` line points at, finalize confirms against,
   and the plan's done criteria enforce mechanically.
