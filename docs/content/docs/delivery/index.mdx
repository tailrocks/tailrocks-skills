...
| `DONE` | Every plan row terminal, the goal condition met, **and the last verification round found no blocking defect** | `tailrocks-reconcile`, and nothing else |
...
no blocking defect, that writes the status.
...
| [`tailrocks-prove`](/docs/skills/tailrocks-prove) | Execution as evidence | Writing status; fixing what it finds | `verification/NN-report.md` — a verdict per statement and per plan row |
| [`tailrocks-reconcile`](/docs/skills/tailrocks-reconcile) | Truth | Writing or refreshing plans | Evidence-backed statuses, the item's Remaining, nothing else |
...
An executor is pointed at the file rather than fed a block:
...
`NOT EXECUTED` with the obstacle, never as passing. It also audits the item's
...
finding that a *skill* let the divergence through routes to
...
observable statements — from the blocking defects, and writes the status a
round earned. A round with a blocking defect sends the item back to
`IN EXECUTION` with that defect as the remaining work; a round with none is the
only way to `DONE`. It writes statuses and Remaining, and nothing else.
...
| A loop that stopped, a repository that moved, or a round to close | `tailrocks-reconcile` |
