...
1. **Select the mode.** Classify as `write`, `review`, or `audit`. `write`
   mutates the approved scope; `review` reports prioritized findings and
   mutates only what the user approves; `audit` is strictly read-only and
   cites findings by file and line.
