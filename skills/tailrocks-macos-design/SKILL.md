...
|---|---|---|
| [`design-principles.md`](references/design-principles.md) | scoring | Apple's principles supply the **axes** but no formula for combining them, so they cannot be the whole review; the platform-citizenship checklists supply the **pass/fail tests**. |
| [`native-behavior.md`](references/native-behavior.md) | approving the brief | **Do not draw what the system can own.** A hand-drawn equivalent forfeits material updates, accessibility adaptation, shape grammar, window states, keyboard behavior, focus indication, and localization. |
| [`macos-craft.md`](references/macos-craft.md) | density, typography, color, iconography, or interaction decisions | Apple's first macOS best practice is *"more content in fewer nested levels"*, against iPhone's *"limit the number of onscreen controls"* — use fewer nested levels for primary content and reserve progressive disclosure for rare controls. |
| [`motion.md`](references/motion.md) | adding or reviewing any animation | A duration-based easing curve cannot preserve velocity through an interruption, so any interruptible, gesture-driven, or retargetable motion built on one is a defect by Apple's own criterion. Dismissal reverses the reveal along the same axis; no animation may be one a person sits through twice; under Reduce Motion every blur — including every glass morph — becomes a fade. |
| [`exemplars.md`](references/exemplars.md), then [`reference-corpus.md`](references/reference-corpus.md) | building taste that persists | Apple's own apps split into Mac-shaped and iPhone-shaped, and the iPhone-shaped ones running on the Mac are the counter-examples — copy the first group only. Every repeated rejection becomes a documented anti-pattern or a rubric line, or the same failure returns next feature. |
