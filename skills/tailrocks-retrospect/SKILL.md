...
1. **Bind one item and its evidence.** One item is one folder: resolve the
   ...
   SHA. **Bind the item at one SHA and record it**; where the package's
   ingest line pins a different one, record both, because the gap between
   them is the item changing under a frozen package. Tabulate every commit:
   SHA, author date, subject, trailers, changed paths. An item whose lane
   carries no `Tailrocks-Skill` trailer at all is refused, not reconstructed
   from subjects: the absent marking is the finding to report.
   **Complete when:** the item, package, rounds, topics, and the full commit
   table exist, or the invocation is declined with what was missing.
...
3. **Run the detectors.** Read
   ...
   Re-open the cited artifact for every finding that will survive into the
   record.
