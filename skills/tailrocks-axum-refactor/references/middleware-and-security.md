...
load-shed, and other fallible layers so the service keeps producing responses
...
4. Catch panics at the outer boundary; still treat them as defects.
...
7. Compress only eligible responses; guard secrets against compression side
   channels.
...
when request identity/tracing must also cover 404/405 responses; document the
choice.
