...
description: >-
  Use only when the user explicitly requests this skill. Audit one skill or every skill in the repository against the authoring doctrine. Read-only: reports defects by layer — description, router, references, evals, wiring, overlap — each with a stable finding ID and its named fix. Never edits.
