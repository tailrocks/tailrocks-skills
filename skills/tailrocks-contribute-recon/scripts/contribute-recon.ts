...
async function loaderSkill(): Promise<string> {
  ...
  const skill = path.dirname(path.dirname(entrypoint));
  if (
    args.length !== 2 ||
    args[0] !== "--skill-file" ||
...
if (import.meta.main) {
  try {
    ...
    process.exit(await contributionStageCli("recon", process.argv[1]!, skillFile));
  } catch (error) {
    process.stdout.write(
      `${JSON.stringify({ schema: "tailrocks.contribution-stage/v1", stage: "recon", outcome: "refused", code: "invalid_input", contribution_id: "", repository: "", head: "", actions: [], mutations: [], partial_state: [], recovery_artifacts: [], runtime: null, detail: error instanceof Error ? error.message : "bootstrap refused" })}\n`,
    );
    process.exit(2);
  }
}
