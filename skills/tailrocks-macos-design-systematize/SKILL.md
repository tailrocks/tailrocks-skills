...
1. Bind the exact approved screen/prototype revision, live user sign-off,
   independent passing review hash, current product corpus root, allowed paths,
   and existing bytes. Refuse self-approval, failed/stale review, missing
   rendered states, ambiguous ownership, or a request to edit this installed
   skill's references.
...
4. Stage the complete product-owned postimages and publish sequentially by
   expected-preimage-to-owned-postimage CAS. On failure, restore a preimage only
