...
3. **Write the spec.** Read
   ...
   scenarios, screen contracts per mockup. **A screen with a visual
   surface and no blessed design reference stops planning here** — say
   which screens, name the medium's design skill, and let the user run it
   or record the deferral; a schematic mockup is layout intent, never
   pixel truth, and a plan that treats it as truth hands the implementer
   an argument instead of a contract.
   **Complete when:** every `S#`, `F#`, `W#`, `N#`, `B#` lands in the spec
   or a logged deferral; every screen contract cites a blessed reference
   or the user's recorded deferral; `D#`/`R#`/`A#`/`Q#` resolve per the ledger's
