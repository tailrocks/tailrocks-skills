...
`tailrocks-grpc-review`, and stop. Refuse requested mutation or evolution, name
`tailrocks-graphql-best-practices`, and stop. A target that cannot resolve to an
exact diff, path set, or public API revision is refused pending scope. Every
...
unless refusal names exactly one current owner and reason. Never emit a second
free-form findings list, and never mutate while producing any outcome.
