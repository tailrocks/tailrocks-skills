...
description: >-
  Use only when the user explicitly requests this skill. Cold-start a repository audit: fan out category findings, verify each by re-reading its evidence, prioritize by leverage, and turn selected ones into roadmap items or plans. Read-only on source.
argument-hint: "[quick|deep|branch|next|<category>|ask <question>|plan <description>|execute <slug>|sweep] [target]"
