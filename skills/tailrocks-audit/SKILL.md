...
argument-hint: "[quick|branch|next|<category>|ask <question>|plan <description>|execute <slug>|sweep] [target] [--deep] [--batch]"
...
2. **Fan out the audit lanes.** Read
   ...
   **The interface lanes carry no taste of their own.** They judge against
   ...
   frames, and `tailrocks-macos-design`'s review rubric /
   `tailrocks-liquid-glass`'s acceptance gate — never a fresh aesthetic call.
...
6. **`execute` mode: dispatch and review.** Read
   [`references/execution-loop.md`](references/execution-loop.md). Requires a
