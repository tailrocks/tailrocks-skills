...
(from `tailrocks-macos-design`'s prototype step 2, or during a roadmap item's
...
4. `.swift-format` and `.swiftlint.yml` from
   [`swift-format.json`](templates/swift-format.json) and
   [`swiftlint.yml`](templates/swiftlint.yml) — and name the strict
   invocations. [`Tests.swiftlint.yml`](templates/Tests.swiftlint.yml) lands
...
Every file above has a template in [`templates/`](templates/) named after its
...
additionally read [`rust-core.md`](references/rust-core.md). Two rules carry
