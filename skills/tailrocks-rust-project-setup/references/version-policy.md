...
The templates are the only exact pin source for this skill, and each number
lives in exactly one of them:

- `templates/rust-toolchain.toml` owns the Rust channel.
- `templates/Cargo.toml` owns `workspace.package.rust-version` and every
  workspace dependency pin.
- `templates/clippy.toml` owns `msrv`.
- `templates/mise.toml` owns every cargo tool version and the binstall pin.
...
`scripts/resolve-crate-versions.ts` reads these registries and reports what is
stable today; it is what the scheduled refresh runs.
...
When scaffolding or auditing a project that is not this repository, the same
rule applies to its pins: resolve current stable, replace the values in the
artifacts together with the generated lockfiles, and never leave a version in
