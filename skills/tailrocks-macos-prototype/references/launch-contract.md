...
| `--tr-reduce <list>` | `transparency`, `motion`, comma-joined | Forces the reduction substitutions through the app's own accessibility hooks, per process. This previews the substitution; it does not replace the real-settings matrix, which is `tailrocks-macos-visual-qa`'s lane. |
...
The harness view (`templates/ProtoMain.swift`) implements the contract
once; prototypes fill in scenarios and content:
...
- **Determinism before readiness.** Wipe the app's defaults domain at
  ...
  the visual-qa harness waits for it, and geometry must be stable when it
  appears.
...
altered. Per-process overrides preview the design cheaply; the visual-qa
