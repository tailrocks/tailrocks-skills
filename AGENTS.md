...
checks every skill and manifest, and rejects five things in shipped
skill content: code-forge URLs, design-file tool names, model brand
names (a skill names a capability role, never the route that fills it
today), a `SKILL.md` that links into its own `references/` or
`templates/` tree without the resolution-base line, and a `SKILL.md`
link target that starts with `./` or `skills/` (the spec form is
`references/x.md`). The naming gates allow a line that names the thing
